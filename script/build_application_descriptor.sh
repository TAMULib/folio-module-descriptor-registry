...
main() {
  ...
  local output_path_name="application"
...
build_app_desc_build_file() {
  ...
  while [[ ${i} -lt ${total} ]] ; do
    ...
    build_app_desc_print_debug "Processing id=${id}, name=${name}, version=${version} at index ${i} of ${total} for input file: ${file}"
...
build_app_desc_load_environment() {
  ...
  if [[ $(echo -n ${descriptor_name} | grep -sho '[/\]') != "" ]] ; then
    echo "${p_e}The descriptor name may not contain '/' or '\' characters."

    let result=1
    return
  fi

  if [[ ${descriptor_name} == "" ]] ; then
    echo "${p_e}The descriptor name is empty ('/' and '\' characters are not allowed in the name)."
    echo

    let result=1
  fi

  if [[ ${BUILD_APP_DESCRIPTOR_OUTPUT} != "" ]] ; then
    files=${BUILD_APP_DESCRIPTOR_OUTPUT}
  fi
  ...
  if [[ ${BUILD_APP_DESCRIPTOR_OUTPUT_NAME} != "" ]] ; then
    output_path_name=$(echo -n ${BUILD_APP_DESCRIPTOR_OUTPUT_NAME} | sed -e 's|[/\]||g')
  fi
